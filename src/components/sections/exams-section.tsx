  ...
  return (
    <section className="relative w-full py-20 sm:py-28 bg-white overflow-hidden" id="exames">
      ...
      <div className="absolute inset-0 z-0 overflow-hidden pointer-events-none">
        <div className="absolute top-[30%] right-[-10%] h-[350px] w-[350px] rounded-full bg-secondary/5 blur-[90px]" />
        <div className="absolute bottom-[10%] left-[-10%] h-[300px] w-[300px] rounded-full bg-primary/5 blur-[80px]" />
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 sm:gap-16 items-center">
          ...
          >
            ...
            <h2 className="text-3xl font-extrabold tracking-tight text-primary sm:text-5xl leading-tight">
              ...
              <span className="text-secondary bg-clip-text">Laboratório Próprio</span>
            </h2>
            ...
            <ul className="mt-8 space-y-5 w-full">
              {categories.map((category) => (
                <li key={category.title} className="flex items-start gap-3">
                  <CheckCircle2 className="h-5.5 w-5.5 text-secondary shrink-0 mt-0.5" />
                  <div>
                    <h4 className="text-base font-bold text-primary">{category.title}</h4>
            ...
            <div className="mt-10 flex flex-wrap gap-4 w-full sm:w-auto">
              <Link
                href="/exames"
                className="group inline-flex w-full sm:w-auto items-center justify-center gap-2 rounded-full border-2 border-primary bg-transparent hover:bg-primary px-7 py-3 text-sm sm:text-base font-bold text-primary hover:text-white transition-all duration-300 shadow-md hover:-translate-y-0.5 active:translate-y-0"
              >
                Ver todos os exames
          ...
          >
            ...
            <div className="relative w-full aspect-square max-w-[420px] sm:max-w-[450px]">
              ...
              <div className="absolute top-0 left-0 w-[82%] h-[82%] bg-slate-100 border-4 border-white rounded-2xl shadow-lg flex flex-col items-center justify-center text-center p-6 select-none transition-transform hover:scale-[1.01] duration-300">
                <Microscope className="h-8 w-8 text-secondary mb-2" />
                <span className="text-xs font-bold text-primary tracking-wide">INFRAESTRUTURA</span>
              ...
              <div className="absolute bottom-0 right-0 w-[70%] h-[70%] bg-slate-200/80 border-4 border-white rounded-2xl shadow-xl flex flex-col items-center justify-center text-center p-6 select-none transition-transform hover:scale-[1.02] duration-300">
                ...
                <span className="text-xs font-bold text-primary tracking-wide">TECNOLOGIA</span>
